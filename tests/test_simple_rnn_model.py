...
class TestSimpleRNNModel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Dummy data shape: (num_samples, timesteps, features)
        self.X_dummy = np.random.rand(10, self.timesteps, self.features).astype(np.float32)
        self.y_dummy = np.random.randint(0, 2, 10).astype(np.float32)
        self.model = SimpleRNNModel(input_shape=self.input_shape)
        self.save_path = "./test_simple_rnn_model_saved"
    ...
    def test_model_instantiation(self):
        ...
        # Keras Input layer shape includes batch size as None
    ...
    def test_model_save_and_load(self):
        ...
        # Load the model
        loaded_model_instance = SimpleRNNModel.load(self.save_path)
